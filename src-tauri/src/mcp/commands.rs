...
pub async fn mcp_connect(
  ...
  log::info!("[MCP] Starting connection to server: {} with type: {}", name, config.r#type);
  ...
  match config.r#type.as_str() {
    "stdio" => {
      log::info!("[MCP/stdio] Initializing stdio connection for server: {}", name);
      ...
      let build_cmd = || {
        ...
        if cmd_name == "npx" || cmd_name == "npm" || cmd_name == "node" {
          log::debug!("[MCP/stdio] Setting up environment for {} command", cmd_name);
          ...
          if !env_checker.verify_tool_availability(&cmd_name) {
            log::warn!("[MCP/stdio] Tool {} not found in current PATH, attempting to refresh environment", cmd_name);
            ...
            if let Ok(()) = EnvironmentSetup::new().setup() {
              ...
              log::warn!("[MCP/stdio] Failed to setup environment for {} command", cmd_name);
            }
          } else {
            ...
            for raw in &args[start..] {
              if is_path_like(raw) {
                ...
                match tokio::fs::metadata(raw).await {
                  Ok(metadata) => {
                    log::debug!("[MCP/stdio] Path exists: {} (type: {:?})", raw, metadata.file_type());
                  }
                  Err(e) => {
      ...
      log::debug!("[MCP/stdio] Final command details: cmd='{}' args={:?} envs={}", cmd_name, &config.args, config.env.as_ref().map(|v| v.len()).unwrap_or(0));
      ...
      match first {
        Ok(Ok(service)) => { 
          log::info!("[MCP/stdio] First connection attempt successful for server: {}", name);
        ...
        Ok(Err(e)) | Err(e) => {
          ...
          if cmd_name == "npx" {
            if let Some(pkg) = extract_npx_package(&config.args) {
              ...
              log::info!("[MCP/stdio] Second connection attempt successful for server: {}", name);
    ...
    "sse" => {
      ...
      let service: McpService = match ()
        .serve(transport)
        .await {
          Ok(service) => {
            log::info!("[MCP/sse] SSE service created successfully for server: {}", name);
            service
          }
          Err(e) => {
      ...
      log::info!("[MCP/sse] SSE connection established successfully for server: {}", name);
      Ok(())
    }
    "http" => {
      log::info!("[MCP/http] Initializing HTTP connection for server: {}", name);
      ...
      let service: McpService = match ()
        ...
        .await {
          Ok(service) => {
            log::info!("[MCP/http] HTTP service created successfully for server: {}", name);
            service
          }
          Err(e) => {
      ...
      state.0.insert(name.clone(), service);
      log::info!("[MCP/http] HTTP connection established successfully for server: {}", name);
      Ok(())
    }
    _ => {
...
pub async fn mcp_list_tools(server_name: String, state: State<'_, McpState>) -> Result<Vec<Tool>, String> {
  ...
  let ListToolsResult { tools, .. } = match service.list_tools(Default::default()).await {
    Ok(result) => {
      log::debug!("[MCP] Successfully listed {} tools from server {}", result.tools.len(), server_name);
      result
    }
    Err(e) => {
      log::error!("[MCP] Failed to list tools from server {}: {}", server_name, e);
      return Err(e.to_string());
    }
  };
...
pub async fn mcp_call_tool(
  ...
  log::debug!("[MCP] Calling tool {} on server {} with args: {:?}", tool_name, server_name, args);
  
  let service = state.0.get(&server_name).ok_or_else(|| "Server not found".to_string())?;
  let param = CallToolRequestParam { name: tool_name.clone().into(), arguments: args };
  
  let res = match service.call_tool(param).await {
    Ok(result) => {
      log::debug!("[MCP] Tool {} called successfully on server {}", tool_name, server_name);
      result
    }
    Err(e) => {
      log::error!("[MCP] Failed to call tool {} on server {}: {}", tool_name, server_name, e);
      return Err(e.to_string());
    }
  };
...
pub async fn mcp_list_resources(
  ...
  let res = match service.list_resources(Default::default()).await {
    Ok(result) => {
      log::debug!("[MCP] Successfully listed resources from server {}", server_name);
      result
    }
    Err(e) => {
      log::error!("[MCP] Failed to list resources from server {}: {}", server_name, e);
      return Err(e.to_string());
    }
  };
...
pub async fn mcp_read_resource(
  ...
  log::debug!("[MCP] Reading resource {} from server: {}", uri, server_name);
  
  let service = state.0.get(&server_name).ok_or_else(|| "Server not found".to_string())?;
  let params = ReadResourceRequestParam { uri: uri.clone().into() };
  
  let res = match service.read_resource(params).await {
    Ok(result) => {
      log::debug!("[MCP] Successfully read resource {} from server {}", uri, server_name);
      result
    }
    Err(e) => {
      log::error!("[MCP] Failed to read resource {} from server {}: {}", uri, server_name, e);
      return Err(e.to_string());
    }
  };
...
pub async fn mcp_list_prompts(
  ...
  let res = match service.list_prompts(Default::default()).await {
    Ok(result) => {
      log::debug!("[MCP] Successfully listed prompts from server {}", server_name);
      result
    }
    Err(e) => {
      log::error!("[MCP] Failed to list prompts from server {}: {}", server_name, e);
      return Err(e.to_string());
    }
  };
...
pub async fn mcp_get_prompt(
  ...
  log::debug!("[MCP] Getting prompt {} from server: {} with args: {:?}", name, server_name, args);
  
  let service = state.0.get(&server_name).ok_or_else(|| "Server not found".to_string())?;
  let params = GetPromptRequestParam { name: name.clone().into(), arguments: args };
  
  let res = match service.get_prompt(params).await {
    Ok(result) => {
      log::debug!("[MCP] Successfully got prompt {} from server {}", name, server_name);
      result
    }
    Err(e) => {
      log::error!("[MCP] Failed to get prompt {} from server {}: {}", name, server_name, e);
      return Err(e.to_string());
    }
  };
