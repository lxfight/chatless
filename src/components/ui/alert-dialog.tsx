  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        className={cn(
          "bg-white dark:bg-gray-900 dark:text-gray-100 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-5 rounded-2xl border border-gray-200/80 dark:border-gray-700/70 p-6 shadow-xl duration-200 sm:max-w-md",
          className
        )}
        {...props}
      />
...
// 支持通过 asChild 使用自定义容器（例如 <div>）以避免 <p> 内再嵌套区块元素导致的 HTML 语义错误。
